...
- `INOUT` — вход + выход (принять значение, изменить, вернуть). 
...
### 2.1. Что это
`IN` — входные параметры, которые передаются в процедуру при вызове, и используются в процессе выполнения процедуры. 

### 2.2. Характеристики IN
- значение **передаётся** в процедуру;
- параметр можно использовать, но его **нельзя менять** (только чтение);
- даже если попытаться изменить, изменение **не возвращается** вызывающей программе. 
...
### 3.1. Что это
`OUT` — выходные параметры, которые используются, чтобы **вернуть значение из процедуры**. 
...
- вызывающая программа/сессия получает значение OUT после завершения процедуры. 
...
### 4.1. Что это
`INOUT` — двунаправленные параметры: можно **передать значение в процедуру**, изменить его внутри и **вернуть обратно**. 

### 4.2. Характеристики INOUT
- работает и как вход, и как выход;
- начальное значение передаётся в процедуру;
- внутри процедуры параметр можно менять, и новое значение возвращается вызывающей стороне. 
...
1) Вывести `department_id`, где работает сотрудник, по `id` сотрудника.   
2) Создать процедуру `get_employee_age`: принимает `id` (IN) и возвращает возраст (OUT).   
3) Создать процедуру `decrease_salary`: принимает зарплату (INOUT) и уменьшает её на 10%.   
