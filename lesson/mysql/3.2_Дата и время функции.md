...
- **DATE** — только дата `YYYY-MM-DD`  
  Диапазон: `1000-01-01` … `9999-12-31`. 
- **DATETIME** — дата и время `YYYY-MM-DD HH:MM:SS`  
  Диапазон: `1000-01-01 00:00:00` … `9999-12-31 23:59:59`. 
- **TIMESTAMP** — дата и время, часто используется как “время последнего обновления” (может зависеть от timezone сервера)  
  Диапазон: `1970-01-01 00:00:01` … `2038-01-19 03:14:07` (ограничение Unix time). 
- **TIME** — только время `HH:MM:SS`  
  Диапазон: `-838:59:59` … `838:59:59`.
...
Идея: в разных СУБД — разные функции/операторы.
...
1) Вывести ваш возраст на текущий день **в секундах**.   
   Вариант в MySQL (если есть дата рождения):
   ```sql
   SELECT TIMESTAMPDIFF(SECOND, '1990-01-01', NOW()) AS age_seconds;
   ```
2) Какая дата будет через **51 день**. 
   ```sql
   SELECT DATE_ADD(CURDATE(), INTERVAL 51 DAY) AS plus_51_days;
   ```
3) Отформатировать предыдущий запрос: вывести **день недели** для этой даты (использовать документацию MySQL).  
4) Подключиться к `northwind`: вывести `transaction_created_date` из `inventory_transactions` и столбец `+3 часа`. 
   ```sql
...
5) Вывести текст: `Клиент с id <customer_id> сделал заказ <order_date>` из `orders`  
   – двумя способами: **неявное преобразование** и **явное CAST** (для customer_id использовать `CHAR`, не `VARCHAR`). 
