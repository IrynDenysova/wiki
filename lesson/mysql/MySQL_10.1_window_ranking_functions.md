...
Материалы: презентация и конспект урока. 
...
Это оконные функции, которые **присваивают номер/ранг** каждой строке в рамках окна `OVER(...)` — обычно в заданном порядке `ORDER BY` и (опционально) внутри группы `PARTITION BY`. 
...
Даже если значения одинаковые, номера всё равно разные. 

### 2.2. `RANK()`
Присваивает ранги, но **пропускает номера рангов** при одинаковых значениях.   
...
### 2.4. `NTILE(n)`
Делит строки на **n примерно равных групп** (квантили) и присваивает номер группы `1..n`.   
...
- создание отчётов (например, распределение по квартилям через `NTILE`) 
...
- `ORDER BY` — задаёт порядок в рамках окна, по которому и считается ранжирование 
