...
**Фильтрация данных** — отбор строк/групп по условиям. В SQL есть два похожих оператора, но используются они на **разных этапах** запроса: `WHERE` и `HAVING`. 
...
- НЕ может фильтровать по агрегатам (нельзя `WHERE COUNT(*) > 1`) 

### HAVING
- фильтрует **группы** *после* `GROUP BY`
- может содержать агрегатные функции (`COUNT`, `SUM`, `AVG`, …) 
- `GROUP BY` может быть без `HAVING`, но `HAVING` **не бывает** без `GROUP BY` 
...
- `условие_для_группы` — какие группы оставить. 
...
`HAVING` может быть **медленнее**, чем `WHERE`, потому что применяется **после** группировки и расчёта агрегатов. Поэтому, если часть фильтра можно сделать на уровне строк — делай это в `WHERE`, а условия по агрегатам оставляй для `HAVING`. 
...
7) `LIMIT` — ограничивает количество строк 
...
1) Посчитать основные статистики `AVG/SUM/MIN/MAX` для `unit_cost`.   
2) Посчитать количество уникальных заказов `purchase_order_id`.   
3) Посчитать количество продуктов `product_id` в каждом заказе `purchase_order_id`, отсортировать по убыванию количества.   
4) Посчитать заказы по `date_received`, считая только строки, где `quantity > 30`.   
5) Посчитать суммарную стоимость заказов по датам: `quantity * unit_cost`.   
6) Сгруппировать товары по `unit_cost` и вычислить `AVG` и `MAX` для `quantity`, только для `purchase_order_id <= 100`.   
7) Оставить только строки где `inventory_id IS NOT NULL`:
   - создать `category`: если `unit_cost > 20` → `'Expensive'`, иначе `'others'`
   - посчитать количество продуктов в каждой категории. 
