...
- **Избежание дублирования**: данные о клиенте не повторяются в каждом заказе — хранятся один раз в `customers`.   
- **Удобство управления**: обновил данные в одном месте — обновилось “везде”.   
- **Целостность данных**: связи и ключи помогают сохранять непротиворечивость.   
...
Пример из урока: `employees` ↔ `employee_privileges` (у сотрудника одна привилегия или её нет).   
...
Пример: `customers` → `orders`: один клиент делает много заказов, но заказ принадлежит одному клиенту.   
...
Пример: `orders` ↔ `products` — связь M:N реализуется через `order_details` (в заказе много товаров, и товар встречается во многих заказах).   
...
Пример: в `customers` поле `id` — первичный ключ.   
