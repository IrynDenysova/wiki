...
- [Предикатные методы](#2-предикатные-методы-возвращают-true-false) — `isalpha()`, `isdigit()`, `isalnum()`, `isupper()`, `islower()`
- [Поиск подстроки](#3-поиск-подстроки-в-строке) — `find()`, `index()`, `count()`, `in`
- [Изменение регистра](#4-методы-изменения-регистра) — `upper()`, `lower()`, `capitalize()`, `title()`
- [`replace()`](#5-replace-замена-подстроки) — замена подстроки
- [`split()` и `join()`](#6-split-и-join) — разделение и склеивание строк
- [`strip()`](#7-strip-lstrip-rstrip-убрать-мусор-по-краям) — удаление пробелов по краям
