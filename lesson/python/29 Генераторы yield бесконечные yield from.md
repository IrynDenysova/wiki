...
- [[#0) Что повторили в начале]](#0-что-повторили-в-начале)
- [[#1) Генератор: что это и зачем]](#1-генератор-что-это-и-зачем)
- [[#2) Как создают генераторы]](#2-как-создают-генераторы)
- [[#3) Функция с `yield` и как она работает]](#3-функция-с-yield-и-как-она-работает)
- [[#4) Генераторные функции с параметрами]](#4-генераторные-функции-с-параметрами)
- [[#5) Генератор в `for`]](#5-генератор-в-for)
- [[#6) Бесконечные генераторы]](#6-бесконечные-генераторы)
- [[#7) Метод `close()` у генератора]](#7-метод-close-у-генератора)
- [[#8) `yield from` (делегирование)]](#8-yield-from-делегирование)
- [[#Задание 1 (countdown)]](#задание-1-countdown)
- [[#Задание 2 (про `yield`)]](#задание-2-про-yield)
- [[#Задание 3 (про `yield from`)]](#задание-3-про-yield-from)
- [[#1) Фильтр чисел: кратные 5]](#1-фильтр-чисел-кратные-5)
- [[#2) Квадраты чисел от 1 до n]](#2-квадраты-чисел-от-1-до-n)
- [[#ДЗ 1) Бесконечный генератор Фибоначчи]](#дз-1-бесконечный-генератор-фибоначчи)
- [[#ДЗ 2) Генератор уникальных элементов (с сохранением порядка)]](#дз-2-генератор-уникальных-элементов-с-сохранением-порядка)
