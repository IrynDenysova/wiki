...
- [[#1) Вложенные функции (nested functions)]](#1-вложенные-функции-nested-functions)
- [[#2) Область видимости: Enclosing и правило LEGB]](#2-область-видимости-enclosing-и-правило-legb)
- [[#3) `nonlocal`: изменение переменной внешней функции]](#3-nonlocal-изменение-переменной-внешней-функции)
- [[#4) Замыкание (closure)]](#4-замыкание-closure)
- [[#5) Функция как объект]](#5-функция-как-объект)
- [[#6) Декораторы: идея и шаблон]](#6-декораторы-идея-и-шаблон)
- [[#1) Что произойдёт?]](#1-что-произойдёт)
- [[#2) Сопоставление понятий]](#2-сопоставление-понятий)
- [[#3) Что делает декоратор?]](#3-что-делает-декоратор)
- [[#4) Что делает `@decorator_name`?]](#4-что-делает-decoratorname)
- [[#1) Фабрика функций расчёта НДС]](#1-фабрика-функций-расчёта-ндс)
- [[#2) Калькулятор скидок по категориям]](#2-калькулятор-скидок-по-категориям)
- [[#3) Настроенная функция вывода (custom printer)]](#3-настроенная-функция-вывода-custom-printer)
- [[#4) Декоратор нумерации вызовов]](#4-декоратор-нумерации-вызовов)
- [[#ДЗ 1) Фабрика функций округления]](#дз-1-фабрика-функций-округления)
