...
- [[#1) Где применяются декораторы]](#1-где-применяются-декораторы)
- [[#2) Декораторы для функций с аргументами: `*args` и `**kwargs`]](#2-декораторы-для-функций-с-аргументами-args-и-kwargs)
- [[#3) Важно: возвращайте результат из `wrapper`]](#3-важно-возвращайте-результат-из-wrapper)
- [[#4) `functools.wraps`: зачем нужен]](#4-functoolswraps-зачем-нужен)
- [[#5) Декораторы с аргументами (декоратор-фабрика)]](#5-декораторы-с-аргументами-декоратор-фабрика)
- [[#6) Несколько декораторов: порядок выполнения]](#6-несколько-декораторов-порядок-выполнения)
- [[#8.1 Декоратор `framed`: рамка `=` шириной 40]](#81-декоратор-framed-рамка-шириной-40)
- [[#8.2 Настраиваемая рамка: `@framed(width, symbol="=")`]](#82-настраиваемая-рамка-framedwidth-symbol)
- [[#ДЗ 1) `measure_time`: среднее время за 5 вызовов]](#дз-1-measuretime-среднее-время-за-5-вызовов)
- [[#ДЗ 2) `measure_time(repeats)`: среднее время за N вызовов]](#дз-2-measuretimerepeats-среднее-время-за-n-вызовов)
