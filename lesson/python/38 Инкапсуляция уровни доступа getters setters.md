...
- [[#1) Инкапсуляция — что это и зачем]](#1-инкапсуляция-—-что-это-и-зачем)
- [[#2) Уровни доступа в Python (это соглашения, а не “запрет”)]](#2-уровни-доступа-в-python-это-соглашения-а-не-“запрет”)
- [[#3) Манглирование имён (name mangling)]](#3-манглирование-имён-name-mangling)
- [[#4) Защищённые и приватные методы]](#4-защищённые-и-приватные-методы)
- [[#5) Геттеры и сеттеры (ручной стиль)]](#5-геттеры-и-сеттеры-ручной-стиль)
- [[#6) `@property`: “красивый” интерфейс без get/set в названии]](#6-property-“красивый”-интерфейс-без-getset-в-названии)
- [[#Задание 1: ошибка в `Book.show_title`]](#задание-1-ошибка-в-bookshowtitle)
- [[#Задание 1: верные утверждения про уровни доступа]](#задание-1-верные-утверждения-про-уровни-доступа)
- [[#Задание 2: ошибка в `@property`]](#задание-2-ошибка-в-property)
- [[#1) SecureUSB: безопасная флешка (read + lock/unlock)]](#1-secureusb-безопасная-флешка-read-lockunlock)
- [[#2) SecureUSB: доступ через свойство `data` (`@property`)]](#2-secureusb-доступ-через-свойство-data-property)
- [[#ДЗ 1) BankAccount: владелец, баланс, пополнение/снятие/показ баланса]](#дз-1-bankaccount-владелец-баланс-пополнениеснятиепоказ-баланса)
- [[#ДЗ 2) История операций (read-only `history` через property)]](#дз-2-история-операций-read-only-history-через-property)
