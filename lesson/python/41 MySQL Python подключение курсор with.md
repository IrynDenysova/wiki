...
- [[#1) Зачем использовать БД из Python]](#1-зачем-использовать-бд-из-python)
- [[#2) Библиотеки для MySQL и установка PyMySQL]](#2-библиотеки-для-mysql-и-установка-pymysql)
- [[#3) Подключение к базе данных]](#3-подключение-к-базе-данных)
- [[#4) Курсор: выполнение SQL из Python]](#4-курсор-выполнение-sql-из-python)
- [[#5) Получение результатов: `fetchone`, `fetchmany`, `fetchall`, итерация по курсору]](#5-получение-результатов-fetchone-fetchmany-fetchall-итерация-по-курсору)
- [[#6) Параметризованные запросы (позиционные параметры `%s`)]](#6-параметризованные-запросы-позиционные-параметры-s)
- [[#7) Именованные параметры: `%(name)s` + словарь]](#7-именованные-параметры-names-словарь)
- [[#8) Обработка ошибок (`try/except` и `pymysql.MySQLError`)]](#8-обработка-ошибок-tryexcept-и-pymysqlmysqlerror)
- [[#9) Контекстный менеджер `with` для соединения и курсора]](#9-контекстный-менеджер-with-для-соединения-и-курсора)
- [[#10) Задания с урока — ответы (коротко)]](#10-задания-с-урока-—-ответы-коротко)
- [[#11) Домашнее задание — идеи решений]](#11-домашнее-задание-—-идеи-решений)
